...
## Le fil — ordre des incréments, état au 7 septembre 2026
...
| 12 | EVOL limites du modèle dynamique | `EVOL_limites-modele-dynamique_v1` (gelé en session 24, **trois avenants** : 31 août, 31 août, 1er septembre) | **atterri**, 0.1.22 (session 24) | **demande du chef de projet du 30 août 2026, à l'ouverture de la session 24**, sur la section 3 publiée en 0.1.21 : après `section3.etape`, une courte sous-section (un `h3`, deux ou trois `p`, bilingue, **aucune règle CSS, aucun script**) qui dit les **limites connues et non cachées** du modèle dynamique — et qu'elles tiennent **quel que soit le SGBD** (SQL Server, PostgreSQL, IBM i…), parce qu'elles viennent du choix de déduire le schéma de la **première ligne** au lieu de le demander à la base. Deux contraintes : la table visée doit contenir **au moins un enregistrement** ; un **NULL en première ligne** empêche de typer la colonne. Un piège : un NULL sur une colonne **numérique** d'une ligne suivante devient **0 en silence** (spike M-13 du 20 août 2026, `NOTES_CONTENU_la-solution_v16.md`, sept cas, Mono sans pilote réel — la représentation du NULL sur la vraie source n'a pas été observée, à dire comme réserve, pas comme fait). Contre-mesure en une phrase : chaque base sait décrire ses tables (`DSPFFD` sur IBM i, `\d` sous PostgreSQL, `sp_help` sous SQL Server, `INFORMATION_SCHEMA.COLUMNS` partout) et des zones qui savent dire « absent ». **Rembourse [W20]** — son remède (« une phrase de légende ») est remplacé par cette sous-section. Pas un tour du marché des SGBD : les grandes lignes **Atterri en 0.1.22 le 1er septembre 2026 (session 24, merge `e616623`), 356/356, SHIP 0 FAIL après trois revues ; [W20] remboursée, [W40] à [W44] nommées.** Marqué atterri le 1er septembre 2026, à l'ouverture de la session 25 — et non à l'atterrissage, [W24] à l'œuvre |
| 12 bis | EVOL « Déploiement et référentiel » | `EVOL_deploiement-et-referentiel_v1` (gelé le 1er septembre 2026, session 25) | **atterri**, 0.1.23 (session 25) | **Deux ajouts de texte, décision du chef de projet du 1er septembre 2026 (session 25), à la relecture de la maquette d'habillage** (`Etude_Technique/MAQUETTE_habillage_v1.html`, hors dépôt). Les trois lignes `12 bis`, `12 ter`, `12 quater` sont un interlude d'un jour né de la même maquette ; elles **passent dans cet ordre, avant la ligne 13**, et gardent leur numéro pour ne recaler aucun renvoi (option arrêtée le 1er septembre 2026 : 19 renvois « ligne 13/14/15 » dans ce fichier auraient bougé). (a) **La section 3 ne dit pas que le prototype est sorti de l'atelier** : après `section3.etape.p1` (« je ne suis pas allé plus loin » — l'étape qui restait est la requête enregistrée, pas le déploiement) et avant `limites`, un **bloc dépliable** sur le motif de la boîte à outils (`details.boite`, replié, « Pour les curieux : le déploiement, le prototype hors de l'atelier ») : l'API déployée via Web Deploy depuis Visual Studio sur un serveur IIS, interrogée depuis l'extérieur ; connexion à l'IBM i par un profil dédié dont le programme initial, un CL, charge la liste de bibliothèques. **Le mécanisme, jamais les valeurs** (P1). Le bloc pourra grandir sans allonger le fil de lecture — mais il dit ce que *ce* déploiement a fait, pas comment fonctionne IIS. (b) **La section 5 invite dans le dépôt du site et tait que celui de la méthode est privé** : une phrase après `preuve` — « Le référentiel de la méthode, lui, reste privé : ce site montre comment le harnais fonctionne, pas sa mécanique interne. » Trois clés par langue, aucune règle CSS, aucun script. **Préalable de l'habillage** : le chapeau « prototype testé en réel » de la section 3 s'appuie sur (a) **Atterri en 0.1.23 le 1er septembre 2026 (session 25, merge `ce8981d`), 356/356, SHIP en un tour, 0 FAIL, 4 WARN.** Les quatre réserves partent ouvertes et **ne portent aucune dette numérotée** — trois sont des textes de commentaire rendus faux par l'insertion (le commentaire « le paragraphe `preuve` vient de l'annoncer » n'a plus `preuve` pour voisin ; celui de `.boite` ne connaît qu'un client quand la classe en a deux) et un `id="deploiement-titre"` que rien ne référence encore, posé pour le jour où le bloc grandit ; **elles se ferment au premier incrément qui rouvre ces fichiers**. La quatrième est de fond et **appelle un arbitrage** : « le référentiel de la méthode reste privé » précède de peu le lien « La méthode, en détail, sur son propre site », et la distinction **dépôt privé / site public** n'est pas dite — à trancher, ou à laisser. Marqué atterri le 1er septembre 2026, à la clôture, geste manuel du chef de projet ([W24]) |
| 12 ter | EVOL « Habillage » | `EVOL_habillage_v1` (gelé le 1er septembre 2026, **deux avenants**) | **atterri**, 0.1.24 (session 26) | **Le chef de projet trouve le site « trop austère » (1er septembre 2026) ; diagnostic : le contrat de design promet trois registres, deux sont à peine visibles, le registre IA n'a jamais été construit** (`css/styles.css` : `--color-s36`, `--color-api`, aucun jeton IA, aucun sombre). Maquette validée le jour même, **six gestes et pas un de plus** : (A) un **chapeau de registre** en Plex Mono au-dessus de chaque `h2` de `main` — valeurs arbitrées : « S/36 · IBM i » (décor, problème), « API REST · .Net · prototype testé en réel » (solution — porté par le bloc Déploiement ; la formule longue à 58 caractères passait sur deux lignes à 390 px, la courte tient sur une), « API REST · .Net · R&D » (mini-langage : le troisième niveau de la ligne 8 rendu visible), « IA · Harnais TWAIM » (méthode), « IBM S/36 · RPG II » (annexe, valable dans les deux langues) ; (B) l'**accroche en bandeau** sur `#edf5ff` avec le filet gauche des cadres ; (C) le **rythme** : plus de filet gris entre sections, `main h2` à 1,75 rem ; (D) les **cadres de code** sur `--color-bg-soft` ; (E) le **registre IA** en section 5, à deux voix — consigne en Plex Sans sur clair, trace de l'agent en Plex Mono sur `#262626`, **seul sombre du site** ; six jetons avec leur ligne de contraste (mesurés : `#6929c4` 7,74:1 sur blanc, `#f4f4f4` 13,76:1 sur `#262626`) ; contenu = **artefacts réels et committés cités mot pour mot** (la première phrase du contexte du prompt de l'incrément 12 ; deux lignes de `git log` — le premier commit de sa branche et le merge `e616623`), **à valider par le chef de projet avant le gel** ; (F) pied de page avec filet haut, `.disclaimer` enfin réglée. **Rembourse [W18] et [W45].** **Duplications FR = EN voulues et déclarées ([W25])** : `section1.chapeau`, `section2.chapeau`, `annexe.chapeau`, `section5.dialogue.consigne`, `section5.dialogue.trace`. **Refusé le même jour** : les lettres striées IBM sur l'accroche (marque déposée, et le contrat : « les quarante ans d'écart se disent dans le texte, pas dans le costume graphique ») ; un bleu marine (quatrième teinte). **Interdit à cet incrément** : tout ce que porte la ligne 13 et qui n'est pas nommé ici — pas d'harmonisation au passage. La mesure VoiceOver gagne deux objets (huit) : le `p` de chapeau avant chaque titre, le `pre` région du bloc IA **Atterri en 0.1.24 le 1er septembre 2026 (session 26, merge `fa8a8a4`), 356/356, SHIP en trois passes, 0 FAIL, 10 WARN.** [W18] et [W45] remboursées. **Deux avenants, tous deux nés de la passe iPhone du chef de projet** : l'**avenant 1** retire `text-transform: uppercase` de `.chapeau` — la règle transformait « IBM i » en « IBM I » sur un site dont le sujet **est** IBM i ; l'**avenant 2**, groupé, fait le même geste sur `.dialogue .voix`, qui capitalisait `prompts/v0.1` et `git log` dans le seul registre dont le contrat exige des artefacts cités mot pour mot, et recale les deux mentions du prompt devenues fausses. Mesuré : Plex Mono étant à chasse fixe, les six largeurs sont **identiques** avant et après, à 320 comme à 390 px — le remède était gratuit. **Trois dettes partent ouvertes** : (a) à **320 px**, le chapeau de la section 3 passe sur deux lignes dans les deux langues (334,56 px FR et 277,45 px EN pour 273 de colonne) — le critère du prompt n'était tenu qu'à 390 px, aucun débordement de page, `width: fit-content` contient le retour ; à trancher avec le texte du chapeau, jamais en passant ; (b) l'**énumération §B de l'avenant 2** révoque la l. 314 en substance sans la nommer, quand l'avenant 1 nommait la l. 184 — asymétrie dans un fichier committé d'un dépôt public, un avenant 3 la fermerait pour une ligne, **arbitrage rendu : non, la règle d'arrêt prime** ; (c) l'**étiquette de voix du bloc IA** est dans le `code`, sans séparateur : un copier rend `Claude Code · git log5e31650 docs(prompt): …` collé — à joindre à la mesure VoiceOver, qui vise déjà ce `pre`. **Une leçon écrite** : une garde se restreint à son objet, elle n'appauvrit pas le texte qu'elle surveille (le même piège de grep deux avenants de suite). Marqué atterri le 1er septembre 2026 à la clôture, geste manuel du chef de projet ([W24]) |
| 12 quater | EVOL « Coloration C# » | `EVOL_coloration-csharp_v1` (gelé le 1er septembre 2026, session 25, palette arbitrée, **avenant 1** du 1er septembre 2026) | **atterri**, 0.1.25 (session 27) | **Demande du chef de projet du 1er septembre 2026 : « récupérer la colorisation de Visual Studio » pour les trois extraits C# de la section 3.** Contrainte mesurée : `applyI18n` écrit par `textContent` — aucune balise ne peut vivre dans le dictionnaire ; et zéro dépendance, donc pas de bibliothèque. Voie : un module `js/coloration.js`, **une fonction pure** de découpage en jetons (commentaire, chaîne, mot-clé d'une liste fermée, PascalCase) rejouée à chaque `i18n:applied`, **invariant** : la concaténation des jetons est le texte, caractère pour caractère ; suite de tests neuve avec comptes dérivés mécaniquement du dictionnaire (code1 : 3/6/36/16 ; code2 : 1/0/12/10 ; code3 : 8/0/29/23) et preuve de morsure. Simplification assumée : type et membre, une seule couleur. **Palette arbitrée le 1er septembre 2026 — « VS brun »** : mots-clés `#0000ff`, types `#0e6e8c` (Visual Studio clair, recalés AA sur `#f4f4f4`), **chaînes `#8b4513`**, et **commentaires `var(--color-ink-soft)` (#525252) par l'avenant 1 du 1er septembre 2026** — le vert `#0a7a0a` de VS clair est révoqué : sous **deutéranopie** il rejoignait la chaîne à **ΔE 3,3**, plus serré que le ΔE 5,5 qui avait motivé l'arbitrage lui-même (mesure Viénot-Brettel-Mollon par l'espace LMS, refaite indépendamment par le reviewer ; une matrice 3×3 sur RGB linéaire donne ici un résultat faux et avait égaré la première mesure). Critère neuf posé par l'avenant : **aucune paire de jetons sous ΔE 15** en vision normale, protanopie et deutéranopie. Le motif est mesuré, pas esthétique : la chaîne de VS *clair* `#a31515` est à **3,4° de teinte** du rouge de refus `#a2191f` du mini-langage (ΔE CIE76 **5,5**) — une couleur, deux sens sur la même page ; la chaîne de VS *sombre* `#d69d85`, échantillonnée au pixel sur une capture d'éditeur fournie par le chef de projet, porte la teinte juste (49,0°) mais ne rend que **2,12:1** sur `#f4f4f4`. `#8b4513` garde cette teinte (57,2°) à une clarté qui tient sur fond clair : **6,45:1**, **ΔE 28,0** avec le refus. **Le sombre est refusé pour les cadres de code** (même jour) : la palette VS sombre native passerait pourtant AA sur le `#262626` déjà posé (`#569cd6` 5,13:1, `#4ec9b0` 7,43:1, `#d69d85` 6,50:1, `#6a9955` 4,54:1) — ce n'est donc pas la lisibilité qui tranche mais le contrat : `--color-bg-ia-dark` est le **seul sombre du site** depuis `12 ter`, et c'est ce qui rend le registre IA visible ; un deuxième sombre dépenserait le signal du premier. **Asymétrie assumée, non subie** : mesuré sur la maquette, 61/23/60 `span` sur les trois extraits de la section 3 contre **0** sur `#mini-classe` — la section 3 est le coin du développeur .Net, qui reconnaît sa palette ; la section 4 montre à tout lecteur une classe fabriquée sous ses yeux, et le texte brut dit qu'elle sort de la machine, pas d'un éditeur. Les variantes « maison » et « VS clair intégral » sont retirées du prompt **Atterri en 0.1.25 le 2 septembre 2026 (session 27, merge `3ffd135`), 382/382, SHIP en trois passes, 0 FAIL, 10 WARN.** L'avenant 1, **né de la revue et non de l'écran**, révoque le vert des commentaires : sous deutéranopie la chaîne et le commentaire tombaient à ΔE 3,3, plus serré que le 5,5 qui avait motivé l'arbitrage lui-même ; `--code-commentaire` prend `var(--color-ink-soft)`, la paire remonte à 45,9. Trois passes de revue, la troisième imposée par le commit de fil `ca621dd` arrivé après la deuxième — la garde a fait son travail. **Dix dettes nommées, [W48] à [W57].** Marqué atterri le 2 septembre 2026, à la clôture, geste manuel du chef de projet ([W24]) |
| 12 quinquies | EVOL « Finitions » | `EVOL_finitions_v1` (gelé le 3 septembre 2026, session 27 Cowork) | **atterri**, 0.1.26 (session 29) | **Quatre retouches vues sur le site publié, 3 septembre 2026** : la boîte du déploiement en trois points ; un fond vert pâle sous les deux messages du mini-langage (jeton `--color-bg-api-soft` : le registre API gagne son fond clair, comme S/36 et IA ont le leur ; le jaune et le bleu pastel sont pris) ; le bouton « ville du client (jointure) » renommé « (autre fichier) » et son aide réécrite en nommant les deux DURAND ; le sommaire du menu en rangées à filet de registre avec le chapeau de chaque section (maquette `Etude_Technique/MAQUETTE_menu_v3.html`, variante A retenue contre un panneau gris et un panneau brun clair, hors dépôt). **Rembourse [W46] et [W47].** Passe avant la ligne 13, qui interdit l'harmonisation au passage. Le même jour, hors incrément : la liste d'interdits passe de 16 à 38 règles (`gh`, `npx`, `python -m pip`, `ssh`, `WebFetch`/`WebSearch`, écriture dans `.claude/`, `.git/`, `~/.claude/`), et `RD-062` (liste d'autorisations) est ouverte au référentiel. **Atterri en 0.1.26 le 3 septembre 2026 (session 29, merge `8f41458`), 382/382, SHIP en une passe, 0 FAIL, 8 WARN.** [W46] et [W47] remboursées. Le merge a été fait à la main par le chef de projet : `/land` prescrit un `git merge` que la liste d'interdits refuse depuis la veille, et l'agent a refusé de contourner ([W68]). Trois des huit WARN viennent du prompt lui-même (un `padding-right` en doublon, deux renvois « l. NNN » faux dès le commit qui rembourse [W46], un commentaire de `.about` déclaré vrai et devenu faux) : **dix dettes nommées, [W59] à [W68]**, dont deux hors incrément ([W67] Playwright hors de la liste, [W68] `/land`). Marqué atterri le 3 septembre 2026 à la clôture, geste manuel du chef de projet ([W24]) |
| 12 sexies | CHORE sécurité | `CHORE_securite-mcp-et-land_v1` (écrit le 4 septembre 2026, session 28 Cowork) | **atterrie** sur `main`, 7 septembre 2026, 0.1.26 inchangée (essai 0, aucun `/land`) | **Rembourse [W67] et [W68], voie (a) arbitrée le 4 septembre : Cowork écrit, le chef de projet commite, Claude Code ne fait que l'essai 0.** **L'objet premier, dit par le chef de projet le 4 septembre : qu'en mode automatique l'agent ne reparte pas dans une boucle sans fin** (le 3, une vérification de rendu s'est étirée sur une dizaine d'appels, coupée à la main). Un interdit ne compte rien ; le seul frein mécanique sans code est le `ask`. Donc : **une règle `deny` de plus** (`mcp__playwright__browser_run_code_unsafe`, aucun usage légitime ; **39**) et **une règle `ask`** sur tout le serveur (`mcp__playwright`) : chaque appel au navigateur demande au chef de projet, même en mode auto, et un `ask` prime sur tout `allow`. `browser_evaluate` reste disponible, sous ce frein (arbitrage du 4 septembre : Playwright est pratique, le chef de projet voit le JavaScript avant qu'il parte). Le serveur s'appelle `playwright`, lu dans `settings.local.json`, pas deviné ; `browser_navigate` **reste permis, trou nommé** au `CLAUDE.md` (seul chemin réseau, aucune règle ne sait borner une URL). **Le frein ne compte rien : le compteur mécanique est un *hook* `PreToolUse`, au menu — RD-063 au référentiel, [W69] ici.** `/land` **perd son merge** : précondition « branche déjà dans `git branch --merged main` », sinon REFUS avec la commande à taper ; bump `patch` tant que la version est < 1.0.0 ; la pré-garde « clôture poussée » passe par `--first-parent --no-merges` pour laisser passer le merge du chef de projet. Plancher machine `PLANCHER_settings-machine_v3.json` (37 `deny` + 1 `ask`) à poser à la main. **Doc relue le 4 septembre** : un `deny` MCP nu retire l'outil du contexte — la preuve de l'essai 0 est l'**absence** de l'outil, pas un message de refus ; le `ask`, lui, se voit : une question à l'écran au premier `navigate` ; et une faute de frappe dans un nom MCP est **silencieuse** (les noms à `_` échappent à l'avertissement), d'où la liste mot pour mot en ÉTAPE 1. L'outil réseau (`browser_network_request*`) n'entre qu'après lecture de son nom exact. **Même famille que [W58]** (ligne 14 : deux commandes pointent vers le vide) — une commande qui prescrit ce qu'un autre texte interdit — mais non absorbée : [W58] reste à la ligne 14. **Le gabarit du kit** (`TWAIM_Kit/_TEMPLATE_AGENTS/.claude/commands/land.md`) porte le même merge l. 22 : à corriger par le chef de projet après son `git diff` (fichier modifié depuis le 13 août, non enregistré), dépôt privé. Le kit n'a **ni gabarit `settings.json`, ni section « Règles de sécurité »** (mesuré le 4 septembre) : un nouveau projet part sans liste — nommé, pas au programme. `/land` réécrit reste **non éprouvé** jusqu'au prochain atterrissage réel. **Essai 0 joué le 7 septembre 2026** (session Claude Code neuve, mode auto, plancher machine v3 posé le matin par `poser-plancher.js`) : `browser_run_code_unsafe` **absent** de la liste d'outils de l'agent, ses 23 voisins présents (onzième `deny` éprouvée, la première par l'absence) ; `ask` chargé sans avertissement et **question à l'écran à chacun des 2 appels** (`navigate`, `snapshot`), alors que `settings.local.json` porte 4 `allow` sur ces outils : le `ask` prime, comme la doc le dit ; 2 appels, aucune écriture hors `.pipeline/ESSAI0_mcp.md`, arbre propre. **Fait neuf** : le serveur Playwright refuse `file:` de lui-même ; `browser_navigate` atteint le réseau, pas le local. **Nom exact de l'outil réseau**, lu dans le rapport : `mcp__playwright__browser_network_request` ; avec `browser_file_upload` et, à la marge, `browser_network_requests`, à examiner pour un `deny` par le chef de projet, pas au programme. **Fait de méthode** : un `ask` approuvé est invisible depuis la place de l'agent, sa preuve est à l'écran du chef de projet |
| 12 septies | EVOL « Les coulisses » | à rédiger | à venir, **après** `12 sexies` et avant le 1.0.0 | **Arbitré le 3 septembre 2026 au soir avec le chef de projet** : le site est construit sous le harnais et ne le montre pas — la section 5 fait trois paragraphes et un lien, le registre IA n'a qu'un dialogue. Montrer **deux ou trois artefacts réels et committés** dans le registre à deux voix (un extrait de prompt gelé, une réserve du `reviewer` mot pour mot, une leçon datée), **sans chiffre publié**. **Règle de partage, dite par le chef de projet : on montre des instances, jamais des invariants.** Un prompt, une revue, une leçon sont des sorties ; le gabarit, la table §8.1, le contrat du `reviewer`, l'ordre des étapes, le backlog R&D sont la doctrine et restent au référentiel privé. La ligne de partage est le mot « pourquoi » : un texte qui explique pourquoi une règle existe est doctrine ; un texte qui montre ce qui s'est passé peut être en vitrine. **Question ouverte, la seule qui compte : quels trois artefacts.** Passe après la carte Open Graph (Finitions 2) pour que le lien s'affiche proprement **Arbitrages du chef de projet, 9 et 10 septembre 2026** : trois sous-titres, un artefact chacun, dans l'ordre la porte, la machine, l'humain ; **un seul cadre sombre**, celui qui existe, les trois citations neuves en ligne dans la prose ; les trois artefacts sont des **sorties** : la garde chiffrée d'un prompt gelé (27 août, `EVOL_annexe-s36_v1`), un refus du `prompt-reviewer` cité mot pour mot au journal (session 31, 8 septembre), une leçon datée (24 août). **Le contrat du `reviewer`, d'abord retenu comme deuxième artefact, a été refusé le 10 septembre par le `prompt-reviewer` (`BLOCK`, C1) au titre de la règle de partage ci-dessus, et écarté** : la règle tient, le prompt plie. « Sans chiffre publié » vise les **mesures du site** (nombre de tests, de prompts, de leçons, dates de mesure) ; les faits datés d'un récit (« deux annoncées, quatre réelles ») restent : ce sont des événements, pas des compteurs. Prompt `EVOL_les-coulisses_v2` (révision 1 refusée, sortie du dépôt) |
| 12 octies | CHORE prompt-reviewer | `SPIKE_piege-prompt-reviewer_v1` (l'essai 0 ; l'agent et la garde sont dans `.claude/`, écrits le 4 septembre 2026, session Cowork du 4 septembre) | **atterrie** sur `main`, 7 septembre 2026, 0.1.26 inchangée (essai 0, aucun `/land`) | **Le prompt franchit une porte avant de s'exécuter** (RD-060, Core §4.1 v2.32). Agent `.claude/agents/prompt-reviewer.md`, en lecture seule, appelé par `/ship` en ÉTAPE 0 avant tout commit : **C1** contradiction avec `CLAUDE.md`, la liste d'interdits ou une commande → `BLOCK` ; **C2** prérequis chiffrés remesurés ; **C3** lignes §8.1 recalculées depuis le contenu (**rembourse la porte que RD-059 demandait**) ; **C4** périmètre fermé et `docs(prompt):` exact. Verdict `.pipeline/prompt-review.json`, fraîcheur par `sha256` ; refus propre sans effet de bord ; journal `.pipeline/prompt-reviews.log`. **Verrou des trois** (idée du chef de projet) : trois refus sur un sujet → `/ship` s'arrête, session neuve. Voie (a) : Cowork écrit, le chef de projet commite. **Essai 0, session neuve, quatre lancements du même prompt piège** : `/ship prompts/v0.1/SPIKE_piege-prompt-reviewer_v1.md` — attendu à chaque fois `BLOCK`, C1 FAIL (npm install, curl, git push, /land qui merge), C2 FAIL (`mcp__playwright` = 2, pas 5), C3 FAIL (dépendance et réseau non déclarés), C4 FAIL (`docs(prompt)` pointe `_v2`, pas de section « ne fait pas ») ; `git status --short` inchangé, `git branch --list` = `main` seul ; au **quatrième** lancement : `TROIS REFUS SUR SPIKE_piege-prompt-reviewer — on ne révise plus, on découpe, en session neuve`. Le cas positif (un vrai prompt qui passe `SHIP`) s'éprouve au prochain incrément réel. `/fix` n'a pas encore la garde : à faire suivre. `CLAUDE.md` épingle toujours « v2.29 » : re-pin v2.32 au prochain incrément. **Essai 0 joué le 7 septembre 2026**, session neuve, mode auto : **4 `/ship`, 3 `BLOCK`** (11:28, 16:03, 16:08) rendus par trois relecteurs neufs sur le même `sha`, **8 `fails` identiques** (C1 ×4 : `npm install`, `curl`, `git push`, `/land` qui merge ; C2 : `mcp__playwright` = 2 et non 5 ; C3 : quatre lignes §8.1 non nommées ; C4 ×2 : `_v2`, pas de « ne fait pas »), **verrou au quatrième** (« TROIS REFUS SUR SPIKE_piege-prompt-reviewer — on ne révise plus, on découpe, en session neuve »), relecteur non appelé, journal resté à 3 lignes ; aucune branche, aucun commit, `STATUS` intact. Le relecteur **remesure** (deux prérequis justes validés, le faux attrapé) et vérifie le renvoi de ligne (`.chapeau` l. 423, pas 141). **Limite nommée par le relecteur** : la carence de 72 h d'un paquet ne se vérifie pas depuis le dépôt (réseau fermé), ce contrôle reste au chef de projet. **R&D nommée** : le compteur des trois ne distingue pas trois révisions de trois relances du même fichier ; le `prompt_sha256` porte l'information. Reste : le cas positif (`SHIP`) au prochain incrément réel ; `/fix` sans garde |
| 12 nonies | EVOL « Finitions 2 » | `EVOL_finitions-2_v5` (gelé le 8 septembre 2026, session 31 Cowork ; révisions 1 et 2 périmées par deux arbitrages sur l'image, révisions 3 et 4 **refusées par le `prompt-reviewer`**) | **atterrie** sur `main`, 8 septembre 2026, **0.1.27** (merge `1ed4f2a`, session 31, 395/395, SHIP en une passe, 0 FAIL, 5 WARN) | **Trois objets, aucun recouvrement, aucun changement visible sur la page.** (1) **La carte de partage** : neuf balises `og:*` et `twitter:*` dans le `<head>`, **en anglais seul**, parce que les robots de LinkedIn et de X ne lancent pas le JavaScript et n'atteignent donc jamais la bascule FR/EN. Titre et description **repris mot pour mot** du dictionnaire anglais (`dict.en.site.title`, `dict.en.meta.description`) et **tenus par une porte neuve** (`tests/partage.test.js`, morsure prouvée à la naissance, garde de non-vacuité à neuf balises) : une chaîne recopiée en deux porteurs sans accord est la famille de [W62], on ne la rouvre pas sans la garder. Image `assets/og-card.png`, **1200 × 630**, produite hors dépôt par Cowork le 7 septembre (maquette `Etude_Technique/MAQUETTE_carte-open-graph_v1.html`, polices Plex et jetons de couleur du dépôt, aucune image d'origine externe), déposée non suivie par le chef de projet, enregistrée nommément par l'agent. **PNG et non SVG** : LinkedIn et X ne rendent pas le SVG dans une carte, le format est imposé par le lecteur comme la langue (même arbitrage qu'à la ligne 4, où les dessins ont quitté le SVG). **Arbitrages du chef de projet, 7 septembre** : visuel dédié contre capture de page ; anglais seul ; **les trois registres du site sont annoncés et les trois sont couverts**, S/36 par l'enregistrement positionnel, l'API par le JSON, le harnais par la seconde phrase du chapeau : « Built by one engineer directing AI agents. Every prompt, review and lesson is committed in this repository. » — un fait vérifiable dans le dépôt, jamais une qualité qu'on s'attribue (`STYLE_METHOD` S-2). La première proposition de Cowork rangeait le harnais en mention de pied : écartée par le chef de projet, « c'est justement un point fort du site, montrer le fonctionnement du harnais, pas comment il fonctionne ». Une bande sombre portant trois chemins réels du dépôt a ensuite été proposée puis écartée à son tour, trop dense en vignette ; **aucun chiffre de longueur d'enregistrement sur la carte** (« si les gens se posent des questions au niveau de l'étiquette, c'est raté ») ; « (C# reflection) » sous la flèche, pour qu'un développeur .Net comprenne la pièce du milieu en un mot. Métadonnées de provenance signées retirées du PNG avant dépôt (5758 octets illisibles dans un dépôt public) : le fichier est au bit près celui que la maquette produit. (2) **Les renvois de ligne de `css/styles.css`** : mesurés le 8 septembre par relevé des blocs de commentaire, **six commentaires portent huit renvois, répartis sur sept lignes ; sept renvois sur huit désignent aujourd'hui autre chose**, le seul juste étant celui de `.nav-chapeau`, et il part quand même parce qu'il ne l'est que par chance. [W60] n'en nommait que deux, les deux nés faux en 0.1.26. **Balayage complet arbitré par le chef de projet**, au titre de la leçon du 19 août 2026 (« un défaut déclaré une fois se cherche partout où il peut être ») : un commentaire **nomme** sa cible, il ne la situe plus. Preuve en un nombre : `grep -c 'l. [0-9]' css/styles.css` = **0**. Avec [W59] (un `padding-right` en doublon et un commentaire qui dit que l'état servi ne change pas, alors qu'il a gagné un fond) et [W61] (le commentaire de `.about` faux pour moitié, deux `border: none` mortes) : **[W59], [W60] et [W61] remboursées**. (3) **Re-pin `CLAUDE.md` v2.29 → v2.32**, `MAINTENANCE.md` règle 3, au prochain incrément, et c'est celui-ci. **Premier vrai prompt soumis au `prompt-reviewer`, et il l'a refusé.** 8 septembre 2026, 11:11, verdict `BLOCK`, C1 FAIL, C2 WARN, C3 et C4 PASS, sur la révision 3 : **trois erreurs d'arithmétique du rédacteur**, toutes réelles et toutes vérifiées ensuite (« sept commentaires » pour six, « neuf renvois » pour huit, « six faux » pour sept, et un livrable annonçant huit remplacements pour en prescrire six). Le 7 venait d'un `grep -c`, qui compte des **lignes**, lu comme un compte de commentaires. **Les vingt prérequis chiffrés avaient été remesurés conformes** et le corps du prompt tenait : la porte a attrapé exactement ce qu'elle est faite pour attraper, un nombre juste au mauvais objet, sur le premier prompt réel qui lui était soumis. Refus propre : aucune branche, aucun commit, seuls `.pipeline/prompt-review.json` et `prompt-reviews.log` écrits. **Second `BLOCK` à 11:21 sur la révision 4**, un seul FAIL : le message de commit de l'ÉTAPE 7 annonçait « sept renvois » quand l'incrément en traite huit, sept étant le compte des renvois FAUX. La révision 4 avait corrigé les quatre nombres du corps et laissé celui du message de commit : **une correction déclarée n'est pas une correction balayée**, et l'endroit non balayé était le seul qui aurait publié le nombre faux dans l'historique d'un dépôt public. Quatre `warns` du même passage, tous traités en révision 5 : deux sha de base concurrents dans le même texte, une phrase du prompt qui préjugeait du verdict de sa propre garde, un en-tête qui oubliait le commentaire HTML prescrit avec les balises, et le compte de tests que le relecteur ne peut pas remesurer (`npm test` n'est pas en lecture seule). **Verrou des trois à deux crans sur trois** : un refus de plus et le sujet se découpe en session neuve. Le cas positif (`SHIP`) reste à éprouver, avec la révision 5. **Leçon candidate, à inscrire à `tasks/lessons.md` :** un compte se propage dans un texte par ses reformulations, pas par ses occurrences littérales ; corriger un nombre exige de balayer tous les endroits qui le disent autrement, message de commit et critères d'acceptation compris. Nommés, hors périmètre : `.claude/` (l'agent ne peut pas y écrire, et `ship.md` épingle encore « v2.29 » — geste du chef de projet, voie (a)) ; `og:image:width`, `og:image:height`, `og:image:alt`, `og:locale`, `og:site_name`, `twitter:site`, non arbitrées ; le site TWAIM (autre dépôt) ; la vérification du rendu de la carte chez LinkedIn et X, palier 2 du chef de projet après publication |
| 13 | Mise en scène | à rédiger | à venir | Plex Mono des commandes en ligne, indice de défilement des cadres de code (W23), dettes W5, W8, W12, W13, W29, W30 (W18 remboursée le 1er septembre par l'habillage, retirée de cette ligne le 3), plus les trois commentaires faux de `12 quinquies` ([W59], [W60], [W61]) qui se ferment au premier incrément rouvrant `styles.css` ; **en réserve depuis le diagnostic du 23 août** : plafonner la hauteur du tableau `CDEMST` sur petit écran, qui rapprocherait le message de jointure de son geste pour les dix-huit lignes à la fois — écarté de l'incrément 7 au profit de l'extinction de teinte, à peser contre la lecture du décor d'un seul tenant |
...
- **[W18]** **Remboursée le 1er septembre 2026 par l'incrément « Habillage » (0.1.24)** : `.disclaimer` porte enfin une règle (taille de la mention de marques). Énoncé d'origine conservé ci-dessous. **Remboursée le 1er septembre 2026 par l'incrément « Habillage » : `.disclaimer`
...
- **[W45]** **Remboursée le 1er septembre 2026 par l'incrément « Habillage » (0.1.24)** : `#mini-filtre` déclare fond et encre. Énoncé d'origine conservé ci-dessous. **Remboursée le 1er septembre 2026 par l'incrément « Habillage » : `#mini-filtre`
  ...
  parce qu'il ne déclare pas de `color-scheme` — la dette est dormante, pas absente : un hôte ou un
  navigateur qui impose le sombre la réveille. Remède : `background: var(--color-bg); color: var(--color-ink)`.
  **À rembourser par la ligne `12 ter`, « Habillage ».**

- **[W46]** **Remboursée le 3 septembre 2026 par l'incrément « Finitions » (0.1.26)** : le commentaire nomme sa cible. Énoncé d'origine conservé. **Un commentaire désigne son voisin par sa place, et l'insertion l'a périmé.** L'incrément
  `12 bis` a glissé `section5.prive` entre `<p data-i18n="section5.preuve">` et le commentaire qui le
  suivait — « Le dépôt d'abord : le paragraphe `preuve` vient de l'annoncer. » Vrai sur le fond, faux
  sur la forme : « vient de » ne désigne plus le paragraphe qui précède. Relevé par le `reviewer` le
  1er septembre 2026, **inscrit au fil le 2 septembre**. Remède : le commentaire **nomme sa cible**.
  **Se ferme au premier incrément qui rouvre `index.html` — ligne 13.**

- **[W47]** **Remboursée le 3 septembre 2026 par l'incrément « Finitions » (0.1.26)** : le commentaire de `.boite` n'inventorie plus ses clients. Énoncé d'origine conservé. **Jumelle de [W46], autre fichier, même incrément.** Le commentaire de `.boite`
  (`css/styles.css`) décrit **un** client et son tableau qui défile, alors que la classe en a **deux**
  depuis `12 bis`, le second sans tableau. Un commentaire qui **inventorie ses clients** porte une
  hypothèse sur le voisinage, et le voisinage change à chaque incrément. **Ligne 13.**

- **[W48]** **Le fil écrit « recalés » au pluriel pour une seule valeur.** La ligne `12 quater`,
  recalée par `ca621dd`, compresse la parenthèse de sélection sur deux valeurs : « mots-clés `#0000ff`,
  types `#0e6e8c` (Visual Studio clair, **recalés** AA sur `#f4f4f4`) ». Le code n'en recale qu'**une** :
  `css/styles.css` l. 76 reprend `#0000ff` tel quel (7,81:1, il passait déjà AA), l. 77 recale
  `#2b91af` → `#0e6e8c` (3,31:1 → 5,26:1). Le pluriel fait lire au visiteur du fil que `#0000ff`
  aurait été recalculé, ce qui est faux. Le bloc de commentaire de `:root` énonce, lui, la règle juste.
  **Correction de texte au fil, geste manuel.**

- **[W49]** **`--code-commentaire` n'est plus une valeur mais un alias, et rien ne garde le couplage.**
  L'avenant 1 lui fait lire `var(--color-ink-soft)`, jeton sémantique de « texte secondaire » que
  **35 autres règles** de la feuille lisent aussi. La séparation commentaire/chaîne sous dichromatisme
  est devenue une propriété d'accessibilité **portante**, et elle dépend d'un jeton possédé par un
  autre besoin : le jour où l'encre secondaire bouge, les quatre valeurs écrites sur cette ligne
  (7,10:1 ; ΔE 48,8 / 34,8 / 45,9) deviennent fausses **en silence**. Mesuré : `--color-ink-soft` n'a
  qu'une déclaration, aucun `@media prefers-color-scheme` dans la feuille, et **aucun test ne porte de
  couleur** (`grep` d'hexadécimal dans `tests/` = 0). Un relecteur verrait la divergence — rien de
  mécanique ne la verrait. **C'est le quatrième motif de la porte de palette, ligne 14.**

- **[W50]** **Le plancher ΔE 15 n'est écrit à aucun point d'usage.** Le critère neuf de l'avenant 1
  (« aucune paire de jetons sous ΔE 15, en vision normale, protanopie et deutéranopie ») vit au prompt
  gelé (§D.2) et au fil depuis `ca621dd`. Mais le bloc de commentaire de `:root` énonce la règle de
  sélection **précédente** (« valeurs de VS clair reprises quand elles passent AA sur `#f4f4f4`,
  recalées sinon ») et ne dit rien du plancher. Celui qui touchera un `--code-*` dans la feuille ne
  lira ni le prompt gelé ni le fil ; la paire la plus serrée est type/commentaire à **23,7** sous
  protanopie. **Ligne 14, avec la porte de palette.**

- **[W51]** **Une mesure publiée à six décimales d'un côté et muette de l'autre n'est pas
  reproductible.** L'artefact de mesure spécifie la moitié **LMS** du calcul de dichromatisme
  (Viénot-Brettel-Mollon) à six décimales, et laisse la moitié **Lab** non dite (matrice RGB→XYZ,
  point blanc). Le `reviewer`, en implémentation indépendante, rejoue les douze cellules au dixième
  près **sauf deux** — contrôle bleu/jaune (235,2 publié, 235,147 retrouvé) et motclé/commentaire avant
  avenant (200,6 / 200,548) — écart intégralement expliqué par une matrice sRGB→XYZ à 4 décimales au
  lieu de 7. Aucune décision n'en dépend. Ce qui reste : **une mesure se livre avec toutes ses
  constantes, pas seulement celles qu'on a trouvées intéressantes.** Pratique de rédaction, pas produit.

- **[W52]** **La preuve qui ouvre l'avenant est la seule dont la restriction n'est pas énoncée.** La
  preuve 1 publie « 1 » pour un objet dont la commande citée rend **36** sur le fichier entier
  (`grep -c 'var(--color-ink-soft)' css/styles.css`), la valeur juste étant celle du bloc `--code-*`.
  La restriction est bien celle que l'avenant demande, la valeur est bonne — mais le fait qu'elle soit
  restreinte n'est **pas dit**, alors que le même artefact le dit explicitement quatre lignes plus bas
  pour d'autres `grep`. Dans un incrément dont la leçon centrale est « restreindre la mesure à son
  objet », c'est la preuve d'ouverture qui l'applique sans l'énoncer. **Même famille que [W57].**

- **[W53]** **L'en-tête de `js/coloration.js` déclare une simplification quand il y en a cinq.** Il
  n'annonce que la fusion type/membre. **Quatre formes lexicales C#** ne sont pas gérées et se
  coloreraient à faux : commentaire de bloc, chaîne verbatim, chaîne interpolée, littéral de caractère.
  Zéro occurrence dans les six sources du dictionnaire, et les comptes gelés des tests rougiraient si
  l'une apparaissait — **le canari existe, mais il dirait « les comptes ont changé », pas « forme non
  gérée »**. Hors périmètre de l'avenant, vérifié non traité. **Ligne 14.**

- **[W54]** **L'expression `/g` est partagée au niveau module.** `TOKEN` porte un état mutable
  (`lastIndex`) partagé entre tous les appels. Correct aujourd'hui — aucun chemin de réentrance, et la
  remise à zéro à l'entrée n'est pas décorative. Le déclarer dans la fonction **supprimerait la classe
  de défaut** au lieu de la garder sous surveillance. Risque résiduel : un futur appelant réentrant.
  **Ligne 14.**

- **[W55]** **« Invariant ABSOLU » ne tient que pour une entrée chaîne.** `tokenizeCSharp(42)` rend `[]`,
  donc la concaténation vaut `''` au lieu de `'42'`, **silencieusement** ; `null`, `undefined` et `{}`
  jettent. Le JSDoc déclare `@param {string}` et la précondition est énoncée par le type — c'est le mot
  « absolu » qui la contredit. Aucun appelant réel n'est concerné. Correction de texte. **Ligne 14.**

- **[W56]** **Une couture posée pour la testabilité, refermée à moitié.** `paintTokens` appelle le
  `document` **global** alors que `mountColoration` accepte un paramètre `root` : un futur test DOM
  devrait injecter un global plutôt qu'un paramètre. C'est la porte manquante déclarée en **[W13]**,
  même famille que **[W8]** — la règle est gardée, son site d'appel ne l'est pas. **Ligne 14, à
  rembourser du même geste que [W8] et [W13].**

- **[W57]** **La pratique de rédaction des preuves n'est pas acquise.** Les preuves 5 et 6 du prompt
  gelé comptent un littéral et mordent sur le texte qui l'explique — **deux fois dans le même
  incrément**. L'avenant applique le remède à son §C.2 et ses `grep` de palette sont justes (vérifiés
  indépendamment sur `ca621dd`), son §C.1 nomme la ligne qu'il révoque, ce que la dette (b) de `12 ter`
  reprochait à l'avenant précédent. Ce qui reste ouvert n'est donc **pas le produit mais la pratique** —
  et [W52], née le même jour dans un autre artefact, montre qu'elle n'est pas encore tenue.
  **C'est ce que l'agent `prompt-reviewer` de RD-060 surveillerait.**

- **[W58]** **Deux commandes prescrivent la relecture de sections qui n'existent pas.**
  `.claude/commands/session-start.md` l. 14 ordonne de « relire au moins les sections **Sécurité,
  Qualité et Anti-patterns** du `CLAUDE.md` » ; `.claude/commands/ship.md` l. 29 invoque à son tour les
  « anti-patterns ». Mesuré le 2 septembre 2026, sur les quinze titres du fichier **et** par recherche
  de mot : « Qualité » et « Anti-patterns » n'y figurent **nulle part** — zéro occurrence — et
  « Sécurité » n'existait pas avant ce jour, la section « Règles de sécurité » ayant été créée le
  2 septembre. Depuis l'installation des commandes, deux d'entre elles pointent donc **vers le vide**,
  et **silencieusement** : un agent qui obéit ne trouve rien, et rien ne le lui fait dire. Deux remèdes
  possibles, à trancher, jamais les deux : écrire les sections manquantes, ou corriger les commandes.
  **Même famille que RD-059, un cran plus bas** — là où RD-059 dit qu'une table obligatoire n'est pas
  consultée, ici une prescription **désigne une cible inexistante**. Nommer n'est pas réparer :
  cette ligne enregistre, elle ne corrige rien. **Ligne 14.**

- **[W59]** **Remboursée le 8 septembre 2026 par `12 nonies` (0.1.27)** : la déclaration en doublon est retirée et le commentaire réécrit, `.statut` posant déjà le fond du registre API et le padding des deux états. Énoncé d'origine conservé. **`.statut.refuse` porte un `padding-right` en doublon exact** de celui que `.statut` pose
  au même commit (0.1.26), et son commentaire dit « l'état servi ne change pas » alors que l'état servi
  vient de gagner un fond. Le doublon vient du prompt, qui a prescrit la déclaration sans relire la
  règle voisine. **Ligne 13**, avec [W60] et [W61] : trois commentaires de `styles.css`.

- **[W60]** **Remboursée le 8 septembre 2026 par `12 nonies` (0.1.27), et élargie** : les **six** commentaires de `css/styles.css` qui situaient leur cible ont été réécrits, les **huit** renvois retirés, y compris le seul encore juste. Preuve en un nombre : `grep -c 'l. [0-9]' css/styles.css` = **0**. Énoncé d'origine conservé. **Deux renvois « l. NNN » faux à la naissance, au commit qui rembourse [W46].** Les deux
  commentaires neufs du sommaire situent leur cible par un numéro de ligne relevé sur `main` avant
  l'incrément ; le commit ajoute 54 lignes au-dessus et les décale. Leçon du 1er septembre appliquée au
  HTML, oubliée dans le CSS du même prompt. Remède : nommer la cible (`#decor, #probleme, #annexe`),
  jamais la situer. Le `reviewer` propose une porte Vitest qui refuse tout « l. NNN » dans un
  commentaire commité (R&D, non construite). **Ligne 13.**

- **[W61]** **Remboursée le 8 septembre 2026 par `12 nonies` (0.1.27)** : le commentaire dit maintenant ce que la règle fait, et les deux `border: none` mortes sont retirées. Énoncé d'origine conservé. **Le commentaire de `.about` est faux pour moitié** : il annonce un filet de séparation
  repris quand le même commit pose `border-top: none`, et deux `border: none` sont mortes. Le prompt
  disait « son commentaire reste vrai » sans l'avoir relu. **Ligne 13.**

- **[W62]** **La table registre → couleur vit en deux porteurs** que rien ne tient d'accord : les
  sections (`#decor, #probleme, #annexe`) et les liens du sommaire (`a[href="#decor"]`…). Une ancre
  renommée laisserait le filet retomber sur `currentColor`, en silence. Porte à écrire ou porteur
  unique à construire. **Ligne 14.**

- **[W63]** **La paire `.about a` sur le gris neuf ne publie pas sa ligne de contraste**, et franchit
  AA de 0,048 (#0f62fe sur #f4f4f4 : 4,55:1). Renvoie à la porte de palette committée, proposée à la
  revue de `ca621dd`, non construite. **Ligne 14.**

- **[W64]** **Le nom accessible des six liens du sommaire concatène titre et chapeau**, par un espace
  qui n'est pas dans le balisage mais dans le calcul de `flex`. À trancher à [W28], sur lecteur
  d'écran réel : la mesure VoiceOver porte désormais **neuf objets**.

- **[W65]** **Deux des six rangées du menu portent le même chapeau** (« S/36 · IBM i », décor et
  problème) : pour un tiers du sommaire, la ligne ajoutée n'ajoute rien. Se règle avec le texte des
  chapeaux, jamais en passant (même arbitrage que le chapeau de la section 3 à 320 px).

- **[W66]** **La base corrigée `chapeau"` = 24 ne vit que dans un artefact non commité**
  (`.pipeline/changes.md`). Le prochain prompt qui relèvera ses bases repartira du nombre faux (18)
  s'il lit le prompt gelé au lieu du dépôt. Règle : une base se mesure le jour même, jamais reprise
  d'un prompt précédent.

- **[W67]** **Remboursée le 7 septembre 2026 par le CHORE sécurité (`12 sexies`, 0.1.26 inchangée)** : `deny` sur `browser_run_code_unsafe` et `ask` sur tout le serveur `playwright`, tous deux éprouvés par l'essai 0 (`ESSAI0_mcp.md`). Reste nommé, pas remboursé : `browser_navigate` (réseau), `browser_network_request`, `browser_file_upload`, sous le seul `ask` ; le bornage par URL demande le hook [W69]. Énoncé d'origine conservé. **Playwright est hors de la liste d'interdits.** Les outils MCP du navigateur
  (`browser_navigate`, `browser_evaluate`, `browser_run_code_unsafe`…) atteignent n'importe quelle
  URL et exécutent du code arbitraire, en mode automatique, sans aucune demande — pendant que la
  liste ferme `curl`, `wget`, `ssh` et retire `WebFetch`. Mesuré le 3 septembre 2026 : 38 règles,
  aucune ne contient `mcp__`. Vérifié dans la doc officielle le même jour : une règle `deny` sait
  viser un outil MCP entier ou un serveur entier (`mcp__playwright`), **jamais ses arguments** — une
  règle `mcp__…(…)` est ignorée au chargement. Le filtrage par URL passerait par un *hook*. **Prompt
  CHORE dédié, avec [W68] et `RD-062`.**

- **[W68]** **Remboursée le 7 septembre 2026 par le CHORE sécurité (`12 sexies`)** : `/land` ne merge plus et refuse une branche non fusionnée ; la contradiction entre `CLAUDE.md` et `land.md` est levée, et le relecteur la cite désormais comme règle (C1 du 7 septembre). `/land` réécrit reste non éprouvé jusqu'au prochain atterrissage réel. Énoncé d'origine conservé. **`/land` prescrit un `git merge` que la liste d'interdits refuse** depuis le 2 septembre
  2026. Mesuré le 3 septembre au premier atterrissage réel depuis la liste : refus à l'ÉTAPE 2, marqueur
  `LANDING` remis en `READY` par l'agent, merge fait à la main par le chef de projet, `/land` relancé.
  Deux textes du dépôt se contredisent (`CLAUDE.md` : « le merge est un geste du chef de projet » ;
  `.claude/commands/land.md` : l'agent merge). Arbitrage recommandé : `/land` perd son merge et
  s'arrête avant, comme il s'arrête déjà avant le push. Même famille que [W58] : une commande qui
  prescrit ce qu'un autre texte interdit. **Prompt CHORE dédié.** Note : depuis le 3 septembre, le
  bump d'une branche `feat/*` doit être passé en `bump=patch` tant que le jalon 1 n'est pas fermé.

- **[W69]** **Le frein à clic ne compte rien.** Depuis le 4 septembre 2026, `mcp__playwright` est en
  `ask` : chaque appel au navigateur demande au chef de projet. C'est un frein humain, pas un compteur :
  trois appels ou trente, c'est lui qui compte, et lui qui se lasse. Le compteur mécanique est un *hook*
  `PreToolUse` qui lit le nom de l'outil sur son entrée, tient un compte par session dans `.pipeline/`
  et refuse au-delà de N — un budget d'appels, posé par le prompt ou par défaut. Il pourrait aussi
  borner `browser_navigate` à `127.0.0.1` et `file://`, ce qu'aucune règle ne sait faire ([W67]).
  C'est du code : il se prouve à sa naissance (`ASSURANCE_METHOD` couche A) et vit dans `.claude/`,
  donc hors de portée de l'agent. Inscrit au référentiel, **RD-063**, parce qu'il vaut pour tout
  projet du parc. **Ligne 14.**
